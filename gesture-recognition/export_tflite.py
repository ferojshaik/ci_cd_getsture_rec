...
# Paths (defaults; overridable for MLOps pipeline)
...
def convert_to_tflite(quantize=False, saved_model_dir=None, output_dir=None):
    """
    Load SavedModel and convert to TFLite.
    quantize=True: int8 quantization (smaller, faster, ~same accuracy for this model).
    saved_model_dir: override SAVED_MODEL_DIR (e.g. runs/run_<id>/saved_model).
    output_dir: if set, write .tflite files here instead of PROJECT_ROOT.
    """
    in_dir = Path(saved_model_dir) if saved_model_dir else SAVED_MODEL_DIR
    out_dir = Path(output_dir) if output_dir else PROJECT_ROOT
    converter = tf.lite.TFLiteConverter.from_saved_model(str(in_dir))
    ...
    if quantize:
        ...
        out_path = out_dir / "gesture_model_quant.tflite"
    else:
